...
namespace RPGM.Gameplay
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        void UpdateAnimator(Vector3 direction)
        {
            if (animator)
            {
                animator.SetFloat("WalkX", direction.x < 0 ? -1 : direction.x > 0 ? 1 : 0);
                animator.SetFloat("WalkY", direction.y < 0 ? 1 : direction.y > 0 ? -1 : 0);
                animator.SetFloat("Speed", direction.sqrMagnitude);
                if(nextMoveCommand.x == 0.1f || nextMoveCommand.x == -0.1f ||  nextMoveCommand.y == 0.1f || nextMoveCommand.y == -0.1f)
                {
                    animator.SetFloat("lastMoveX", nextMoveCommand.x);
                    animator.SetFloat("lastMoveY", nextMoveCommand.y);
